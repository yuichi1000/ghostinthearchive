...
# アウトロ設定
PAUSE_BEFORE_DISCLOSURE_MS = 2_000  # 本編末尾 → 開示の間の無音
...
def _create_outro_section(
    disclosure_audio: AudioSegment,
    outro_music_path: Path = OUTRO_MUSIC_PATH,
) -> AudioSegment:
    """AI 開示 TTS とアウトロ音楽（処理済み）を合成する。

    アウトロ音楽はフェード・音量調整済みの静的アセット。
    TTS を音楽の先頭にオーバーレイして返す。
    """
    outro_music = AudioSegment.from_mp3(str(outro_music_path))
    return outro_music.overlay(disclosure_audio, position=0)
...
def generate_podcast_audio(
    ...
    if INTRO_MUSIC_PATH.exists():
        ...
        intro_pause = AudioSegment.silent(duration=1000)
        episode_audio = intro_music + intro_pause + episode_audio
    ...
    if OUTRO_MUSIC_PATH.exists():
        ...
        disclosure_pause = AudioSegment.silent(duration=PAUSE_BEFORE_DISCLOSURE_MS)
        outro_section = _create_outro_section(disclosure_audio)
        episode_audio = episode_audio + disclosure_pause + outro_section
        logger.info(
            "アウトロセクション追加: 開示TTS %.1f秒, アウトロ音楽 %.1f秒",
            len(disclosure_audio) / 1000.0,
            len(outro_section) / 1000.0,
        )
