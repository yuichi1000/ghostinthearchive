...
Librarian → Historian → Storyteller → Visualizer → Publisher.
...
ghost_commander = SequentialAgent(
    ...
    description=(
        ...
        "Librarian → Historian → Storyteller → Visualizer → Publisher の順で実行し、"
        "歴史的ミステリーと民俗学的怪異を調査・分析・コンテンツ化・画像生成・公開する。"
    ),
    sub_agents=[librarian_agent, historian_agent, storyteller_agent, visualizer_agent, publisher_agent],
)
