...
- {visual_assets}: Illustrator のトップ画像アセット（generate_image が返した raw JSON）
...
**重要: `images` フィールドは JSON に含めないでください。**
画像のアップロードと `images` フィールドの設定は `publish_mystery` ツールが内部で自動的に処理します。
...
### ステップ 2: Firestore に保存

構築した JSON を `publish_mystery` ツールに渡して Firestore に保存してください。

`publish_mystery` の呼び出し時には、第2引数 `visual_assets_json` に {visual_assets} の内容をそのまま渡してください。
`publish_mystery` ツールが mystery_id 生成後に画像を自動的に Cloud Storage にアップロードし、
`images.hero` と `images.variants` を Firestore データに自動設定します。
