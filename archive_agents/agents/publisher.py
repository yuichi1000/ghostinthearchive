...
**レスポンシブ画像バリアント対応:**
{visual_assets} に `variants` 配列がある場合、元画像パスに加えてバリアント画像のパスも
すべて `upload_images` に渡してください。例:
```
["元画像.png", "元画像_sm.webp", "元画像_md.webp", "元画像_lg.webp", "元画像_xl.webp"]
```

返却される各画像の `public_url` をステップ 2 で使用します。
...
```json
{
  ...
  "images": {
    "hero": "[lg バリアントの public_url（バリアントがない場合は元画像の public_url）]",
    "variants": {
      "sm": "[sm バリアントの public_url]",
      "md": "[md バリアントの public_url]",
      "lg": "[lg バリアントの public_url]",
      "xl": "[xl バリアントの public_url]"
    }
  },
  "pipeline_log": [],
  "status": "pending",
...
**重要: `images.hero` フィールドにはステップ 1 で `upload_images` が返した lg バリアント（1200px）の `public_url` を設定してください。**
バリアントがない場合は元画像の `public_url` を設定してください。
`images.variants` には各バリアント（sm, md, lg, xl）の `public_url` をマッピングしてください。
バリアントがない場合は `variants` フィールドを省略してください。
画像がない場合は `images` フィールド全体を省略してください。
