...
def _validate_domain_batch(
    ...
    timeout_per_link: float,
    domain_delay: float,
) -> list[tuple[ArchiveDocument, LinkCheckResult]]:
    """同一ドメインのドキュメントを逐次検証する（レートリミット維持）。"""
    results: list[tuple[ArchiveDocument, LinkCheckResult]] = []
    for i, doc in enumerate(docs):
        if i > 0 and domain_delay > 0:
...
def validate_documents(
    ...
    ドメイン別にグルーピングし、異なるドメインは並列で検証する。
    同一ドメイン内は逐次処理してレートリミットを維持する。
    ...
    # ドメイン別にグルーピング（入力順序を保持）
    domain_groups: dict[str, list[ArchiveDocument]] = defaultdict(list)
    for doc in documents:
        hostname = urlparse(doc.source_url).hostname or ""
        domain_groups[hostname].append(doc)

    # ドメイン並列で検証（同一ドメイン内は逐次）
    all_results: list[tuple[ArchiveDocument, LinkCheckResult]] = []
    workers = min(max_workers, len(domain_groups))
    with ThreadPoolExecutor(max_workers=max(workers, 1)) as executor:
        futures = {
            executor.submit(
                _validate_domain_batch, docs, timeout_per_link, domain_delay
            ): domain
    ...
    # 結果を集計（入力順序に依存しない — 除外/保持ロジックは個別判定）
    for doc, result in all_results:
    ...
    logger.info(
        "Link validation: %d/%d reachable, %d removed (%dms, %d domains parallel)",
        reachable_count,
        len(documents),
        len(removed_urls),
        total_ms,
