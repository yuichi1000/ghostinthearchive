...
scholar_tools.py から分離したレガシーファイル I/O 操作。
ローカルファイルからの検索結果読み込みと分析コンテキスト構築を提供する。

注意: 現在のパイプラインではセッション状態経由のデータ受け渡しが主流であり、
これらの関数はレガシー互換および adk web / adk eval で使用される。
"""
