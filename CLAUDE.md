...
Ghost in the Archive - 世界の公開デジタルアーカイブを5つの学術領域（歴史学・民俗学・文化人類学・言語学・文書館学）で多言語横断分析し、記録の隙間に潜む Ghost を発掘する自律型AIエージェントシステム

### 設計思想：Ghost = 記録の隙間に潜むアノマリー

**Ghost の定義**: 世界の公開デジタルアーカイブを多言語横断分析した結果浮かび上がるアノマリー。公開情報だけで仮説を立て、公開情報だけで検証する。それでもなお残る「説明不能な余白」— 不在の中の存在感 — それが Ghost。

**5つの柱**:

1. **自律型AIエージェント** — 人間のバイアスや疲労なき調査
2. **公開情報の透明性** — すべての仮説は公開記録のみで構築・検証可能
3. **多言語横断発見** — 異なる言語の記録を突き合わせて初めて見えるアノマリー
...
- **Storyteller:** claude-sonnet-4-5@20250929 (ブログ記事生成 — Vertex AI 経由 Claude Sonnet 4.5)
